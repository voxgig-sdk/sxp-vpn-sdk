...
import (
	...
	omni "github.com/voxgig-sdk/sxp-vpn-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.SxpVpnError); is {
				out := map[string]any{
					"name":    "SxpVpnError",
...
func omniCtx(arg any, client *sdk.SxpVpnSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.SxpVpnSDK); is {
			client = live
