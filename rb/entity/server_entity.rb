...
class ServerEntity
  ...
  def data_set(args)
    if args
      @_data = SxpVpnHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = SxpVpnHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Array<Server>, Array] the matching Server items; raises SxpVpnError on failure
