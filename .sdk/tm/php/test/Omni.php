...
//    only - a typed SxpVpnContext there would make every ctx
...
final class SxpVpnOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new SxpVpnContext($clean, null);
    ...
    public static function err_from_map($m): ?SxpVpnError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new SxpVpnError(is_string($code) ? $code : '', $msg);
    }
