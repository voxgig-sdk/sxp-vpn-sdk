...
// SxpVpn SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class SxpVpnMakeError
{
    public static function call(?SxpVpnContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new SxpVpnContext([], null);
        }
        $op = $ctx->op ?? new SxpVpnOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new SxpVpnResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof SxpVpnError) ? $err->msg : (string)$err;
        $msg = "SxpVpnSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new SxpVpnError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof SxpVpnError) {
