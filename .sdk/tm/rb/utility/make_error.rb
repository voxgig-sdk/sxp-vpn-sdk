...
module SxpVpnUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = SxpVpnContext.new({}, nil)
    end
    op = ctx.op || SxpVpnOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || SxpVpnResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(SxpVpnError) ? err.msg : err.to_s
    ...
    sdk_err = SxpVpnError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(SxpVpnError)
