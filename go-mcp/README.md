...
[MCP](https://modelcontextprotocol.io) server exposing the SxpVpn SDK as
two agent tools — `sxp-vpn_list` and `sxp-vpn_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/sxp-vpn-mcp)
make build

# 2. Provide credentials via the environment
export SXP_VPN_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user sxp-vpn \
  -- /absolute/path/to/sxp-vpn-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./sxp-vpn-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// sxp-vpn_list: first page of records
{ "entity": "server" }
{ "entity": "server", "query": { } }

// sxp-vpn_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/sxp-vpn-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user sxp-vpn \
     -- "$PWD"/dist/*/sxp-vpn-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `sxp-vpn_list` and `sxp-vpn_load` tools now appear
   in new sessions. Ask the agent to *"list server using sxp-vpn"*
   and it calls `sxp-vpn_list` with `{"entity":"server"}`.
...
```sh
export SXP_VPN_APIKEY=sk_live_xxx            # API key
export SXP_VPN_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `sxp-vpn_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `sxp-vpn_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `SXP_VPN_APIKEY` | API key sent with every request. |
| `SXP_VPN_BASE` | Optional override of the API base URL. |
...
```sh
./sxp-vpn-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"sxp-vpn_load","arguments":{"entity":"server","query":{"id":1}}}}'
```
