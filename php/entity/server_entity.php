...
class ServerEntity
{
    ...
    {
        if ($args) {
            $this->_data = SxpVpnHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = SxpVpnHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   the SDK boundary; throws SxpVpnError on failure (item-5 convention).
     */
