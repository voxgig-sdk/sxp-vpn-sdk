...
class FtCtrl extends SxpVpnControl
{
...
class FtClient
{
    ...
    private SxpVpnUtility $utility;

    public function __construct(SxpVpnUtility $utility, array $options)
    {
    ...
    public function get_utility(): SxpVpnUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (SxpVpnContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (SxpVpnContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = SxpVpnConfig::make_feature($name);
    ...
    private static function buildUrl(SxpVpnSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof SxpVpnError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new SxpVpnResponse($fetched);
        }
    ...
    private function fail(SxpVpnContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (SxpVpnContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
