...
require_once __DIR__ . '/../sxpvpn_sdk.php';
...
{
    private static function utility(): SxpVpnUtility
    {
        return new SxpVpnUtility();
    }
    ...
    private static function ctx(array $over = []): SxpVpnContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new SxpVpnContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new SxpVpnOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof SxpVpnError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new SxpVpnOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = SxpVpnMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new SxpVpnOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = SxpVpnMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new SxpVpnOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = SxpVpnMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new SxpVpnSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = SxpVpnMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new SxpVpnResponse([]),
            'result' => new SxpVpnResult([]),
        ]);
        [, $err] = SxpVpnMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new SxpVpnSpec(['step' => 's']),
            'response' => new SxpVpnResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new SxpVpnResult([]),
        ]);
        [, $err] = SxpVpnMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new SxpVpnSpec(['step' => 's']),
            'response' => new SxpVpnResponse(self::resp(200, ['v' => 1])),
            'result' => new SxpVpnResult([]),
        ]);
        [, $err] = SxpVpnMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new SxpVpnSpec(['step' => 's']),
            'response' => new SxpVpnResponse(self::resp(200, ['v' => 2])),
            'result' => new SxpVpnResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        SxpVpnMakeResponse::call($ctx);
    ...
    {
        $preset = new SxpVpnResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new SxpVpnSpec([]),
            'response' => new SxpVpnResponse([]),
            'result' => new SxpVpnResult([]),
        ...
        [$got, $err] = SxpVpnMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new SxpVpnResult([])]);
        [, $err] = SxpVpnMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new SxpVpnSpec([]), 'result' => null]);
        [, $err] = SxpVpnMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new SxpVpnSpec(['step' => 's']),
            'result' => new SxpVpnResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new SxpVpnOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = SxpVpnMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new SxpVpnSpec(['step' => 's']),
            'result' => new SxpVpnResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new SxpVpnOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = SxpVpnMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new SxpVpnResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new SxpVpnSpec([]),
            'result' => new SxpVpnResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = SxpVpnMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = SxpVpnMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new SxpVpnError('boom', 'boom');
        $utility->fetcher = function (SxpVpnContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new SxpVpnSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = SxpVpnMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (SxpVpnContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = SxpVpnMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(SxpVpnResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (SxpVpnContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new SxpVpnSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = SxpVpnMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new SxpVpnResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new SxpVpnSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = SxpVpnMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = SxpVpnMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = SxpVpnMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new SxpVpnResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, SxpVpnDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new SxpVpnResult(['ok' => false])]);
        $this->expectException(SxpVpnError::class);
        SxpVpnDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new SxpVpnResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', SxpVpnMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new SxpVpnBaseFeature();
        $b = new SxpVpnBaseFeature();
        SxpVpnFeatureAdd::call($ctx, $a);
        SxpVpnFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): SxpVpnBaseFeature
    {
        $f = new SxpVpnBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        SxpVpnFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): SxpVpnContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new SxpVpnSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = SxpVpnPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new SxpVpnResponse(['headers' => null]),
            'result' => new SxpVpnResult([]),
        ]);
        SxpVpnResultHeaders::call($ctx);
