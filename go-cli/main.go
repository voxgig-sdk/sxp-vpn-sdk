// Command SxpVpn-cli is a CLI + REPL for the SxpVpn SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      SXP_VPN_APIKEY (api key), SXP_VPN_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: SXP_VPN_APIKEY carries the API key and
	// SXP_VPN_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("SXP_VPN_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("SXP_VPN_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewSxpVpnSDK(opts)
