...
boru-driven command-line client **and** interactive REPL for the SxpVpn
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/sxp-vpn-cli)
make build
...
./sxp-vpn-cli list server

# 5. Override the API base URL for a single call
SXP_VPN_BASE=https://api.example.com ./sxp-vpn-cli list server
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/sxp-vpn-cli
   ```
...
Run with no arguments to open a REPL (prompt `sxp-vpn>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/sxp-vpn-cli`. |
...
(`./sxp-vpn-cli <expr>`) and interactively (the REPL), and expressions compose the
