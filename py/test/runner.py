# SxpVpn SDK test runner

from __future__ import annotations
import os
import json

from sxpvpn_sdk.utility.voxgig_struct import voxgig_struct as vs


class SxpVpnTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            SxpVpnTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = SxpVpnTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = SxpVpnTestRunner.getenv("SXP_VPN_TEST_LIVE")
        override = SxpVpnTestRunner.getenv("SXP_VPN_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = SxpVpnTestRunner.getenv(key)
        ...
        explain = SxpVpnTestRunner.getenv("SXP_VPN_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if SxpVpnTestRunner._test_control is not None:
            return SxpVpnTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                SxpVpnTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            SxpVpnTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = SxpVpnTestRunner.load_test_control()
...
def load_env_local():
    SxpVpnTestRunner.load_env_local()


def env_override(m):
    return SxpVpnTestRunner.env_override(m)


def entity_data(v):
    return SxpVpnTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return SxpVpnTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return SxpVpnTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return SxpVpnTestRunner.load_test_control()


def live_client_options():
    return SxpVpnTestRunner.live_client_options()


def live_delay_ms():
    return SxpVpnTestRunner.live_delay_ms()
