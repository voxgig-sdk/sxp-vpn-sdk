...
function isRunnable(code) {
    return (/new\s+SxpVpnSDK\b/.test(code) ||
        /\bSxpVpnSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+SxpVpnSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/SxpVpnSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
